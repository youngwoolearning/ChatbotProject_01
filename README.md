...
<img src="chatbotq_EnglishWordBot_Logo.png">
...
| 음식 | 챗봇 |
| 초대할 손님을 위해 만들 음식을 정한다. | 1) 목표 사용자를 위해 만들 챗봇(우리는 영어단어봇)을 정한다. |
| 음식을 할 수 있는 곳(주방)으로 간다. | 2) 챗봇을 만들 수 있는 곳(챗봇빌더 사이트, 단비)으로 간다. |
(필요한 경우) 잠긴 문을 연다. | 3) 챗봇빌더 사이트에 로그인한다. |
| 음식 만드는데 필요한 재료를 모은다. (어떤 재료는 미리 준비해두어야 한다.) | 4) 챗봇 제작에 필요한 요소를 파악한다. (어떤 요소는 미리 준비해두어야 한다. 우리의 경우 단어 문제 자료) |
| 음식을 만든다. | 5) 챗봇을 제작한다. |
| 가까운 사람에게 음식 맛보기를 부탁한다. | 6) 아는 사람에게 챗봇 테스트를 부탁한다. |
| 손님에게 음식을 낸다. (식탁 또는 지정된 장소) | 7) 목표 사용자에게 챗봇을 알린다. (챗봇 링크 전달, 플랫폼에 탑재) |
| 음식에 대한 손님의 피드백에 귀 기울인다. | 8) 챗봇에 대한 사용자의 피드백에 귀 기울인다. |
| 다음에 같은 음식을 할 경우를 위해 개선점을 생각해둔다. | 9) 다음에 챗봇을 업데이트할 경우를 위해 개선점을 생각해둔다. |
